...
export default function AdminDashboardPage() {
  ...
  const [detailData, setDetailData] = useState<Submission | null>(null);
  ...
  const getTipoLabel = (tipo: string) => {
    return tipo === 'entrenador' ? 'Entrenador' : 'Árbitro';
  };

  const getTipoColor = (tipo: string) => {
    return tipo === 'entrenador' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800';
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500">Entrenadores</h3>
            <p className="mt-2 text-3xl font-bold text-blue-600">
              {submissions.filter(s => s.tipo === 'entrenador').length}
            </p>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500">Árbitros</h3>
            <p className="mt-2 text-3xl font-bold text-green-600">
              {submissions.filter(s => s.tipo === 'arbitro').length}
            </p>
          </div>
        </div>
                ...
                {submissions.map((submission, index) => (
                  <tr key={submission.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => handleViewDetail(submission.id)}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getTipoColor(submission.tipo)}`}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{getGenderLabel(submission.genero)}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{submission.tiempo_experiencia} años</td>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-900">{getTipoLabel(detailData.tipo)}</p>
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase">Género</label>
                  <p className="text-gray-900">{getGenderLabel(detailData.genero)}</p>
                </div>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase">Experiencia</label>
                  <p className="text-gray-900">{detailData.tiempo_experiencia} años</p>
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 uppercase">Nivel</label>
                <p className="text-gray-900 whitespace-pre-wrap">{detailData.nivel}</p>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 uppercase">Último Curso</label>
                <p className="text-gray-900">{detailData.ultimo_curso || 'No especificado'}</p>
              </div>
              {detailData.tipo === 'entrenador' && detailData.club_entrena && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase">Club que Entrena</label>
                  <p className="text-gray-900">{detailData.club_entrena}</p>
                </div>
              )}
              <div>
